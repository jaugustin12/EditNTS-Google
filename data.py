...
class Datachunk():
    def __init__(self,data_path):
        ...
        for i in os.listdir(self.data_path):
            directory = os.listdir(self.data_path + '/'+ i)
            for j in directory:
                self.listdir.append(self.data_path + '/'+ i + '/' + j)
#        print(self.listdir)
    ...
    def example_generator(self,shuffle=True):
        while len(self.listdir) != 0:
            ...
            with open(self.data_path + self.listdir.pop(), "rb") as fh:
                df = pickle.load(fh)
    ...
    def batch_generator(self, batch_size=1, shuffle=True):
        while len(self.listdir) != 0:
            # print("reading a new chunk with %d chunks remaining" % len(self.listdir))
            with open(self.listdir.pop(), "rb") as fh:
                df = pickle.load(fh)
            if shuffle:
                df = df.sample(frac=1).reset_index(drop=True)
                # print('shuffling the df')
